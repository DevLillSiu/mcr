...
router.get("/nuoi_instagram", async (req, res) => {
  ...
  try {
    ...
    let results = await query(
      "SELECT id, username, link_save_file FROM mcr_instagram WHERE kt = 0 AND date_reg <= (current_timestamp - interval '3 hours') AND pc_name = $1 AND (status <> 'Nuoixong' OR status IS NULL) LIMIT 1 FOR UPDATE",
      [pc_name]
    );

    if (results.rowCount > 0) {
      const data = results.rows[0];
      await query("UPDATE mcr_instagram SET kt = 1 WHERE id = $1", [data.id]);
      await query("COMMIT");
      return res.json(data);
    } else {
      results = await query(
        "SELECT id, username, link_save_file FROM mcr_instagram WHERE kt = 0 AND pc_name = $1 AND status = 'Nuoixong' AND date_nuoi <= (current_timestamp - interval '24 hours') LIMIT 1 FOR UPDATE",
        [pc_name]
      );
      if (results.rowCount > 0) {
        const data = results.rows[0];
...
router.post("/insert", async (req, res) => {
  ...
  const sqlQuery = `INSERT INTO mcr_instagram (${columnsString}) VALUES (${placeholders})`;
...
router.put("/update", async (req, res) => {
  ...
  let queryString = "UPDATE mcr_instagram SET ";
...
router.delete("/delete", async (req, res) => {
  ...
  const sqlQuery = "DELETE FROM mcr_instagram WHERE id = $1";
