...
router.get('/get_mcr_hotmail', (req,res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query(`SELECT id, username, password FROM mcr_hotmail WHERE status = 0 LIMIT 1 FOR UPDATE`, (error, results) => {
            ...
            db.query(`UPDATE mcr_hotmail SET status = 1 WHERE id = ? `, data.id, (updateError) => {
                if (updateError) {
...
router.post('/insert_mcr_hotmail', async (req, res) => {
...
router.put('/update_mcr_hotmail', async (req, res) => {
...
router.delete('/delete_mcr_hotmail', async (req, res) => {
    ...
    const sqlQuery = 'DELETE FROM mcr_hotmail WHERE id = ?';
