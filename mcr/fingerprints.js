...
router.get("/get", (req, res) => {
  db.query("BEGIN", (err) => {
    ...
    db.query(
      `SELECT id, finger FROM fingerprints WHERE using = 0 AND used < 5 LIMIT 1 FOR UPDATE`,
      (error, results) => {
        if (error) {
        ...
        db.query(
          `UPDATE fingerprints SET using = 1 WHERE id = $1`,
          [data.id],
          (updateError) => {
            if (updateError) {
...
router.post("/insert", async (req, res) => {
  ...
  const sqlQuery = `INSERT INTO fingerprints (${columnsString}) VALUES (${placeholders})`;
...
router.put("/update", async (req, res) => {
  ...
  let queryString = "UPDATE fingerprints SET ";
...
router.delete("/delete", async (req, res) => {
  ...
  const sqlQuery = "DELETE FROM fingerprints WHERE id = $1";
